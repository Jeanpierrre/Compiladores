...
Instruction* Parser::parseInstruction() {
  ...
  // Verificar si hay una etiqueta opcional
  if (match(Token::LABEL)) {
    label = previous->lexema;
    // Remover el ':' al final de la etiqueta
    label.pop_back();
    if (!match(Token::EOL)) {
      cout << "Error: se esperaba fin de línea después de etiqueta" << endl;
      exit(0);
    }
  }

  // Verificar la instrucción
  if (match(Token::POP) || match(Token::ADD)) {
    // Instrucciones que requieren un número como argumento
    ttype = previous->type;
    if (!match(Token::NUM)) {
      cout << "Error: se esperaba un número después de " << Token::token_names[ttype] << endl;
      exit(0);
    }
    ...
    int reg = stoi(previous->lexema);
    instr = new Instruction(label, Token::tokenToIType(ttype), reg);
  } else if (match(Token::GOTO) || match(Token::JMPEQ) || match(Token::JMPGT) ||
             match(Token::JMPGE) || match(Token::JMPLT) || match(Token::JMPLE)) {
    // Instrucciones que requieren un ID (etiqueta) como argumento
    ttype = previous->type;
    if (!match(Token::ID)) {
      cout << "Error: se esperaba un identificador de etiqueta después de " << Token::token_names[ttype] << endl;
      exit(0);
    }
    string targetLabel = previous->lexema;
    instr = new Instruction(label, Token::tokenToIType(ttype), targetLabel);
    ...
    cout << "Error: no se encontró una instrucción válida en " << current->lexema << endl;
    exit(0);
  }

  // Verificar fin de línea
  if (!match(Token::EOL)) {
    cout << "Error: se esperaba fin de línea" << endl;
    exit(0);
  }
